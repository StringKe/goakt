...
description: The spawn recipe for high-churn, connection-lifetime actors.
...
Both concerns are covered by combining two existing [`SpawnOption`](/actor/actor-system) values:
...
<Note>
This recipe does not stop the actor for you. You are expected to call `pid.Shutdown(ctx)` (or send a `Shutdown`/stop message) when the underlying connection closes — that is the whole point: the actor's lifetime is driven by the connection, not by an idle timer.
</Note>
...
2. The node spawns one actor per connection with the recipe above, using the connection ID as the actor name so it can be addressed by other nodes.
...
If your actor is long-lived but must not relocate (e.g. it owns node-local state), `WithRelocationDisabled` alone is enough; the passivation half of the recipe is specifically for the high-churn, externally-driven-lifetime case.
