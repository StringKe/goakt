...
<Note>
Relocation as described on this page requires a graceful shutdown. If a node crashes (kill -9, OOM, etc.), `preShutdown` and `persistPeerStateToPeers` never run, so no peer state is replicated and relocation does not occur by default. See [Crash Relocation](/actor/crash-relocation) for the opt-in placement journal that closes this gap.
</Note>
