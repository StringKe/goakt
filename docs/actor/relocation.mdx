...
<Note>
  Relocation works for both graceful and abrupt departures. On a graceful shutdown the departing node replicates a `PeerState` snapshot to its oldest peers. On a crash (`kill -9`, OOM, network partition) that snapshot never gets written, so the leader instead replays the departed node's [placement journal](/actor/crash-relocation) if one is configured, or otherwise reconstructs the relocation set from the replicated cluster registry. Actors spawned with `WithRelocationDisabled` are still lost with the node either way.
</Note>
...
Relocation does not depend on a clean shutdown. There are three sources for a departed node's relocation set, tried in order:
...
2. **Placement journal (opt-in crash fallback)**: when the leader finds no snapshot and a [placement journal](/actor/crash-relocation) is configured, it replays that node's journaled placements to reconstruct the same `PeerState` a graceful shutdown would have produced. This needs no quiescence wait, so it runs synchronously in the `NodeLeft` handler.
3. **Registry-derived set (crash fallback)**: when neither of the above finds anything, the leader instead reconstructs the set by scanning the replicated cluster registry for every actor and grain still hosted on that node, once olric's post-departure partition repair has gone quiet.
...
- [Crash Relocation](/actor/crash-relocation): Opt-in placement journal for node crashes (kill -9, OOM)
